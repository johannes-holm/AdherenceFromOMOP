...
#'
#' @param df (mandatory) - This should be the table from OMOP CDM drug_exposure with chronic medication events
#' @param person_id (voluntary) - By default the function calculates adherence for all people in the OMOP drug_exposure, who, by definition, are chronic medication consumers. If specified the function calculates adherence for the specific person in the the dataframe. Data type numeric (example: person_id=3)
#' @param cma (voluntary) - By default the function calculates adherence with all different CMA algorithms (CMA1-CMA9). If specified the function can calculate adherence using only necessary algorithm. Data type string (example: cma = 'CMA4' or cma = c('CMA1', 'CMA2'))
#' @param med (voluntary) - By default the function calculates adherence for all chronic medications (chronic medication ingredients). If specified the function can calculate adherence only for specific medication (chronic medication ingredient). Data type string (example: med='1332418'(RxNorm Ingredient id for Amlodipine) or med='19097684,1549786'(RxNorm Ingredient id for gestodene and ethinyl estradiol used in the same medication))
#'
